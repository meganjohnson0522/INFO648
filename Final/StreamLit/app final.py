...
with st.sidebar:
    ...
    st.markdown("Upload the four CSV files required to run the pipeline.")

    f_student   = st.file_uploader("student_tracts_raw.csv",  type="csv")
    f_forecast  = st.file_uploader("forecast_tracts_2020.csv", type="csv")
    f_region    = st.file_uploader("region_tract_counts.csv",  type="csv")
    f_dict      = st.file_uploader("data_dictionary.csv",      type="csv")
...
# ── Main pipeline (runs only when files are ready) ────────────────────────────
if not all([f_student, f_forecast, f_region, f_dict]):
    st.info("👈 Upload all four CSV files in the sidebar to begin.")
    st.stop()

# Load data
student_tracts  = pd.read_csv(f_student)
forecast_tracts = pd.read_csv(f_forecast)
region_counts   = pd.read_csv(f_region)
data_dict       = pd.read_csv(f_dict)
